		...
		for(int i = 0 ; i < sharkList.size(); i ++) {
			...
			for(int d = 1 ; d <= 4 ; d++) {
				...
				if(boundary(nx,ny)) {
					...
					} else if (map[nx][ny].sharkNum == s.sharkNum) {
						mySmellList.add(priorityDirection);
					}
				}
			}
			if(!flag && mySmellList.size() > 0) {
				for(int a = 1 ; a <= 4 ; a++ ) {
					int direction = priority[s.sharkNum][s.direction][a];
					if(mySmellList.contains(direction)) {
						int nx = s.x + dx[direction];
						int ny = s.y + dy[direction];
						tmpMap[nx][ny] = new smell(s.sharkNum,k);
						tmpSharkList.add(new shark(s.sharkNum, direction, nx, ny));
						break;
					}
				}
			}else if(!flag && mySmellList.size() == 0) {
