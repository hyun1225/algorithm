...
public class 백준_3190_뱀 {
	...
	public static void main(String[] args) throws FileNotFoundException {
		...
		for(int i = 0 ; i < directCnt; i ++) {
			int time = sc.nextInt();
			String d = sc.next();
			directList.add(new direcInfo(time, d));
		}
		...
		while(true) {
			...
			for(int i = 0 ; i < directList.size(); i++) {
				if(answer == directList.get(i).time) {
					chanegeDirect(directList.get(i).direction);
					directList.remove(i);
					break;
				}
			}
	...
	private static void chanegeDirect(String direction) {
